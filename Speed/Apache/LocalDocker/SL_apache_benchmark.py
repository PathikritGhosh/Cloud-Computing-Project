...
path='/home/clouduser/benchmarks/apache.tar'
...
def initialize():
	...
	subprocess.run('docker-machine ssh master docker image load -i apache.tar',shell=True,stdout=subprocess.PIPE,stderr=subprocess.PIPE)
	print('Initialization Completed')

def run_process():
	subprocess.run('docker-machine ssh master docker run -i pathikrit/apache',shell=True,stdout=subprocess.PIPE,stderr=subprocess.PIPE)
...
def write_result(response):
	with open("result_apache_%s_%s.txt" %(vCPUs[x],mem[x]), 'w') as txt_file:
